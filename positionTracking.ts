...
const debugLogsOnExtensionChannel = (changes, toUpdateRanges, updatedRanges, outputChannel) => {
   ...
   outputChannel.appendLine('Calculated ranges')
   for (const range of updatedRanges) {
      if (range) {
         outputChannel.appendLine(`    start: ${range.start.line} ${range.start.character}`)
         outputChannel.appendLine(`    end: ${range.end.line} ${range.end.character}`)
 ...
 * @param {boolean} [options.keepRemovedRanges]
 */
const getUpdatedRanges = (
   ...
   let keepRemovedRanges = undefined
   if (options) {
      ;({ onDeletion, onAddition, outputChannel, keepRemovedRanges } = options)
   }
   if (!onDeletion) {
      onDeletion = 'shrink'
   }
   ...
   if (keepRemovedRanges === undefined) {
      keepRemovedRanges = false
   }
   ...
   const updatedRanges = keepRemovedRanges
      ? toUpdateRanges
      : toUpdateRanges.filter((range) => range)
